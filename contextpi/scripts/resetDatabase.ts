...
export const resetDatabase = async () => {
  ...
  try {
    ...
    console.log(`Resetting MongoDB Database: ${dbName}...`);
    ...
    const shipmentsData = Array.from({ length: 35 }, (_, i) => {
      ...
      return {
        ...
        shipmentDate: FIXED_DATE,
        expectedDeliveryDate: new Date("2026-08-27T00:00:00.000Z"),
        actualDeliveryDate: status === "DELIVERED" ? new Date("2026-08-27T00:00:00.000Z") : undefined,
        status: status,
    ...
    console.log("Deterministic database reset complete!");
  } catch (err) {
...
if (process.argv[1] && process.argv[1].includes("resetDatabase")) {
  resetDatabase();
}
